...
// 是交易所服务器的 TCP 入口点，负责接受客户端连接、接收订单请求、验证序列号并将响应路由回交易客户端
// 主要职责：
...
namespace Exchange {
    class OrderManager {
    public:
        OrderManager(ClientRequestLFQueue *clientRequests,ClientResponseLFQueue *clientResponses,
        ...
        auto run() noexcept {   // 该函数在独立线程中持续运行，负责将来自交易系统的响应消息通过 TCP 发送给客户端，并保证消息顺序正确
            ...
            while (run_) {
                ...
                for (auto clientResponse = outgoingResponses->getNextToRead(); outgoingResponses->size() && clientResponse; clientResponse = outgoingResponses->getNextToRead()) {
                    TTT_MEASURE(T5t_OrderManager_LFQueue_read, logger);

                    auto &nextOutgoingSeqNum = cidNextOutgoingSeqNum[clientResponse->client_id_];
                    Common::getCurrentTimeStr(time_str_);
                    logger.log("%:% %() % Processing cid:% seq:% %\n", __FILE__, __LINE__, __FUNCTION__, time_str_,
                                clientResponse->client_id_, nextOutgoingSeqNum, clientResponse->toString());
                    
                    ASSERT(cidTcpSocketMap[clientResponse->client_id_] != nullptr,
                            "Dont have a TCPSocket for ClientId:" + std::to_string(clientResponse->client_id_));
                    
                    START_MEASURE(Exchange_TCPSocket_send);
                    // 先发送序列号，再发送响应体
                    cidTcpSocketMap[clientResponse->client_id_]->send(&nextOutgoingSeqNum, sizeof(nextOutgoingSeqNum));
                    cidTcpSocketMap[clientResponse->client_id_]->send(clientResponse, sizeof(MEClientResponse));
    ...
    private:
        ...
        // 记录下一个即将发出的序列号
        std::array<size_t, ME_MAX_NUM_CLIENTS> cidNextOutgoingSeqNum;

        // 记录下一个期望收到的序列号
        std::array<size_t, ME_MAX_NUM_CLIENTS> cidNextExpSeqNum;

        // // 客户端 socket 映射
        std::array<Common::TCPSocket*, ME_MAX_NUM_CLIENTS> cidTcpSocketMap;
