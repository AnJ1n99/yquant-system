...
MatchingEngine::MatchingEngine(ClientRequestLFQueue* clientRequests,
...
void MatchingEngine::processClientRequest(
    const MEClientRequest* client_request) noexcept {
  // 获取对应symbol的订单簿
...
void MatchingEngine::sendClientResponse(
    const MEClientResponse* response) noexcept {
  Common::getCurrentTimeStr(time_str_);
