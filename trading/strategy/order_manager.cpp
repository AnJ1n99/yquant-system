...
namespace trading {
  ...
  auto OrderManager::newOrder(OMOrder *order, SymbolId ticker_id, Price price, Side side, Quantity qty) noexcept -> void {
    ASSERT(next_order_id_ < kMaxOrderIds, "客户端订单号超出交易所索引范围");
    const exchange::MatchingEngineClientRequest new_request{exchange::ClientRequestType::NEW, trade_engine_->clientId(), ticker_id,
  ...
  auto OrderManager::cancelOrder(OMOrder *order) noexcept -> void {
    const exchange::MatchingEngineClientRequest cancel_request{exchange::ClientRequestType::CANCELED, trade_engine_->clientId(),
