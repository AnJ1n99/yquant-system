...
class TCPServer {
public:
	explicit TCPServer(Logger &logger) : logger_(logger), listener_socket(logger) {
	}

	// start listening for connections on the provided interface and port
	auto listen(const std::string &iface, int port) -> void;

	// check for new connections and or dead connections and update container that track the sockets
	auto poll() noexcept -> void;

	// pubilsh outgoing data to the send buffers and incoming data from the recive buffer
