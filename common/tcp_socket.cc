...
namespace Common {
	// 既用于客户端“连接”，也用于服务端“监听”
	auto TCPSocket::connect(const std::string &ip, const std::string &iface, int port, bool isListening) -> int {
	...
	auto TCPSocket::sendAndRecv() noexcept -> bool {
		...
		iovec iov {inbound_data_.data() + nextRevVaildIndex_, TCPBufferSize - nextRevVaildIndex_}; 
		msghdr msg {&socket_attrib, sizeof(socket_attrib), &iov, 1, ctrl, sizeof(ctrl), 0};
		...
		if (readSize > 0 ) {
			...
			logger_.log("%:% %() % read socket:% len:% utime:% ktime:% diff:%\n", __FILE__, __LINE__, __FUNCTION__,
                  timeStr_, socket_fd, nextRevVaildIndex_, userTime, kernelTime, (userTime - kernelTime));

			recv_callback(this, kernelTime);
		}
		if (nextSendVaildIndex_ > 0) {
			// 非阻塞调用，发送数据
			const auto n = ::send(socket_fd, outbound_data_.data(), nextSendVaildIndex_, MSG_DONTWAIT | MSG_NOSIGNAL);
			getCurrentTimeStr(timeStr_);
			logger_.log("%:% %() % send socket:% len:%\n", __FILE__, __LINE__, __FUNCTION__, timeStr_, socket_fd, n);
		}
