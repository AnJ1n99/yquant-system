...
auto TCPServer::listen(const std::string& iface, int port) -> void {
	...
	ASSERT(addToEpollList(&listener_socket), "epoll_ctl() failed. error:" + std::string(std::strerror(errno)));
}

auto TCPServer::sendAndRecv() noexcept -> void {
    auto recv = false;
    std::for_each(recvSockets.begin(), recvSockets.end(), [&](auto socket) {
    ...
    std::for_each(sendSockets.begin(), sendSockets.end(), [](auto socket) {
        socket->sendAndRecv();
    });
}
...
auto TCPServer::poll() noexcept -> void {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (event.events & EPOLLIN) {
            ...
            logger_.log("%:% %() % EPOLLIN socket:%\n", __FILE__, __LINE__, __FUNCTION__,
                    time_str_, socket->socket_fd_);
            // 没有此 socket
            if (std::find(recvSockets.begin(), recvSockets.end(), socket) == recvSockets.end()) {
    ...
    while (haveNewConnection) {
        ...
        ASSERT(setNonBlocking(fd) && disableNagle(fd), "setNonBlocking() or disableNagle() failed on socket:" + std::to_string(fd));
