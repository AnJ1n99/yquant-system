...
namespace Common {
	...
	auto McastSocket::sendAndRecv() noexcept -> bool {
		...
		const ssize_t n_rcv = recv(socketFd, inboundData.data() + nextRcvValidIndex, McastBufferSize - nextRcvValidIndex, MSG_DONTWAIT);
		if (n_rcv > 0) {
			nextRcvValidIndex += n_rcv;
			getCurrentTimeStr(timeStr);
			logger.log("%:% %() % read socket:% len:%\n", __FILE__, __LINE__, __FUNCTION__, timeStr, socketFd,
                  nextRcvValidIndex);
			recvCallback(this);
		}
		...
		if (nextSendValidIndex > 0) {
			ssize_t n = ::send(socketFd, outboundData.data(), nextSendValidIndex, MSG_DONTWAIT | MSG_NOSIGNAL);
			getCurrentTimeStr(timeStr);
			logger.log("%:% %() % send socket:% len:%\n", __FILE__, __LINE__, __FUNCTION__, timeStr, socketFd, n);
		}
	...
	auto McastSocket::send(const void* data, size_t len) noexcept ->void {
    	...
    	ASSERT(nextSendValidIndex < McastBufferSize, "Mcast socket buffer filled up and sendAndRecv() not called.");
	}
}