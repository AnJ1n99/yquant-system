...
namespace Common {
    ...
    public:
        ...
        // allocate a new object of type T, use placement new to init the object, mark the block as in-use and return the object
        template<class... Args>
        T* allocate(Args&&... args) noexcept { // use perfect farword
            auto objBlock = &(store_[nextFreeIndex]);
            ASSERT(objBlock->isFree, "Expected free ObjectBlock at in index: " + std::to_string(nextFreeIndex));
        ...
        auto deallocate(const T* elem) noexcept {
            // elem is the address of the object to be deallocated, we need to find the index of the block
            const auto elemIndex = (reinterpret_cast<const ObjectBlock*>(elem) - &store_[0]);
            ASSERT(elemIndex >= 0 && static_cast<size_t>(elemIndex) < store_.size(), "Invalid element pointer");
            ASSERT(!store_[elemIndex].isFree, "Expected in-use ObjectBlock at index: " + std::to_string(elemIndex));
